"use client";

import {
  Hero,
  Tech,
  Experience,
  Works,
  Contact,
  About,
  Navbar,
} from "@/components";
import AniGrid from "@/components/AniGrid";
import Percentage from "@/components/Percentage";
import Splash from "@/components/Splash";
import { useState } from "react";
import { GithubIcon, Instagram, TwitterIcon } from "lucide-react";
import { Toaster } from "sonner";
import Link from "next/link";

export default function Home() {
  const [loading, setLoading] = useState(true);

  return (
    <main className=" bg-[url(/grid.svg)] relative  min-h-screen  ">

      <Toaster richColors position="top-center" />

      {loading ? (
        <Splash setFisinshed={() => setLoading(false)} />
      ) : (
        <>
          <AniGrid />
          <div id="main" className=" main z-0 relative overflow-x-hidden pb-4">
            <div
              className="hero bg-cover transition-all  duration-100 bg-no-repeat bg-center 
        relative w-full  max-md:min-h-[50dvh]  p-0    "
            >
              <Percentage />
              <Navbar />
              <Hero />
            </div>
            <About />
            <Works />
            <Tech />
            <Experience />

            <div className=" max-md:hidden p-0 fixed z-50 bottom-[0px] left-4  transition-all  duration-100   ">
              <div className="flex gap-4 flex-col h-full items-center  w-[50px]    ">
                <Link
                  target="_blank"
                  href={"https://www.fiverr.com/younes_mohamed7"}
                  className="flex rounded-full h-[50px] w-[50px] 
                  justify-end items-start  text-neutral-400  flex-1 transition-all  duration-100 
                  shadow-lg
                  hover:scale-125 cursor-pointer    group "
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    height="50"
                    width="50"
                    viewBox="0 0 400 400"
                    className="rounded-full fill-black hover:fill-white "
                  >
                    <circle cx="150" cy="150" r="500" />
                    <g
                      className=" fill-neutral-200   duration-100 group-hover:fill-black"
                      fill="#fff"
                    >
                      <path d="M364.4 162.7c0-6.6-5.2-12-11.8-12-6.4 0-11.7 5.3-11.7 12 0 6.6 5.2 12 11.7 12 6.6.1 11.8-5.3 11.8-12zm-11.8 8.7c-4.5 0-8-3.8-8-8.7 0-4.8 3.5-8.6 8-8.6 4.6 0 8.2 3.8 8.2 8.6 0 4.9-3.6 8.7-8.2 8.7z" />
                      <path d="M355.8 163.7c.6-.2 1.9-1.1 1.9-3 0-2.3-1.5-3.7-4-3.7h-5.3v11.3h3.5v-3.8h.9l1.6 3.8h3.8l-2.1-3.9c-.2-.6-.3-.7-.3-.7zm-3-1.6h-.9v-2.7h.9c.8 0 1.2.4 1.2 1.3.1.9-.4 1.4-1.2 1.4z" />
                      <circle cx="104.6" cy="163" r="9" />
                      <path d="M114 177.9H72.8v-2.7c0-5.3 5.3-5.4 8-5.4 3.1 0 4.5.3 4.5.3v-14.6s-2.8-.4-6.6-.4c-8.6 0-24.5 2.4-24.5 20.6v2.3h-7.5v13.5h7.5V220h-7v13.5H81V220h-8.2v-28.5h22.5V220h-7v13.5H121V220h-7zm70 0h-29.5v13.5h5l-6.4 20c-1.2 3.3-1.5 7.3-1.5 7.3h-.4s-.3-4-1.5-7.3l-6.4-20h5v-13.5h-29.5v13.5h6.2l15.4 42h22l15.4-42h6.2zm54.6 25.5c0-15.4-9.3-26.8-25.8-26.8-17.9 0-28.9 12.7-28.9 29 0 14.8 10.7 29.1 30.5 29.1 15 0 23.9-7.8 23.9-7.8l-6.8-12.9s-7.4 5.3-15.6 5.3c-5.9 0-11.5-3.1-12.9-10.2h35.2c-.1-.1.4-3.9.4-5.7zm-35.2-4.6c1-4.3 3.6-8.2 8.9-8.2 4.1 0 7 3.8 7 8.2zm114.1-8.1h-.2s.2-1.1.2-2.8V185c0-5.1-2.8-7.1-7.9-7.1h-17.5v13.5h5.2c1.5 0 2.4.9 2.4 2.4V220h-7.5v13.5h33.7V220h-7.5v-8.1c0-10.1 5-16.7 15.3-16.7 2.3 0 3.8.3 3.8.3v-18.3s-1.1-.2-2.2-.2c-8.4-.1-15.4 6.1-17.8 13.7zm-49.3 0h-.2s.2-1.1.2-2.8V185c0-5.1-2.8-7.1-7.9-7.1h-17.5v13.5h5.2c1.5 0 2.4.9 2.4 2.4V220h-7.5v13.5h33.7V220h-7.5v-8.1c0-10.1 5-16.7 15.3-16.7 2.3 0 3.8.3 3.8.3v-18.3s-1.1-.2-2.2-.2c-8.5-.1-15.5 6.1-17.8 13.7z" />
                    </g>
                  </svg>
                </Link>

                <Link
                  target="_blank"
                  href={"https://www.instagram.com/younesmohamed_77"}
                  className=" flex justify-end items-start  flex-1 transition-all  duration-100 r
                 hover:text-white text-neutral-400   hover:bg-gradient-to-r
                 shadow-lg
                   hover:scale-125 cursor-pointer   text-3xl"
                >
                  <Instagram className=" h-[35px] w-[35px]  " />
                </Link>

                <Link
                  target="_blank"
                  href={"https://github.com/mohamed-younes16"}
                  className=" flex justify-end items-start text-neutral-400  flex-1 transition-all  duration-100 r
                 hover:text-white hover:bg-gradient-to-r
                 shadow-lg
                   hover:scale-125 cursor-pointer   text-3xl"
                >
                  <GithubIcon className=" h-[35px] w-[35px]  " />
                </Link>
                <Link
                  target="_blank"
                  href={"https://twitter.com/younesmed_77"}
                  className=" flex justify-end items-start text-neutral-400  flex-1 transition-all  duration-100 r
                 hover:text-white hover:bg-gradient-to-r
                 shadow-lg
                   hover:scale-125 cursor-pointer   text-3xl"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    shape-rendering="geometricPrecision"
                    text-rendering="geometricPrecision"
                    image-rendering="optimizeQuality"
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    viewBox="0 0 512 462.799"
                    className=" fill-neutral-400 hover:fill-white h-[35px] w-[35px]   duration-100 group-hover:fill-black"
                  >
                    <path
                      fill-rule="nonzero"
                      d="M403.229 0h78.506L310.219 196.04 512 462.799H354.002L230.261 301.007 88.669 462.799h-78.56l183.455-209.683L0 0h161.999l111.856 147.88L403.229 0zm-27.556 415.805h43.505L138.363 44.527h-46.68l283.99 371.278z"
                    />
                  </svg>
                  {/* <TwitterIcon className=" h-[35px] w-[35px]  " /> */}
                </Link>
                <div className="w-[1px] mt-[10px]  bg-[white] h-[100px]"></div>
              </div>
            </div>
            <div className="relative border-t border-gray-500 mt-12">
              <Contact />
            </div>
          </div>
        </>
      )}
    </main>
  );
}
