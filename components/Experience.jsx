...
const Experience = () => {
  ...
  return (<>
    ...
    <div className="max-w-7xl mx-auto mt-24 px-6" id="work" >
        ...
        className="text-secondary text-lg"> MY Actuall WORK</motion.p> 

        <motion.h3 variants={textVariant(.5)} initial="hidden" whileInView={"show"} 
         viewport={{once:true}} className="h3 font-bold max-md:text-5xl
          max-sm:text-3xl text-7xl  my-8">LIVE PROJECTS <span className=" h-3 w-3 ml-2 inline-block bg-violet-600 rounded-full"></span> </motion.h3>
        <motion.p  className="text-secondary leading-[30px]">
            Following projects showcases my skills and experience through real-world examples of <br  className="max-md:hidden"/> my work.
            Each project is briefly described with links to live demos in <br  className="max-md:hidden"/>it. It reflects my ability to solve problems, 
            ...
            <Tilt   >
                ...
                <div onClick={ev=>handlego(ev,e.source_code_link ? e.source_code_link : "#contact")}
                   ...
                   rel="noreferrer" >
                        ...
                        >
                          {e.next ? (
                            <Image src={next} alt="next image" className=" h-[60px] w-[60px] absolute 
                                                      object-cover text-4xl text-gray bottom-6 animate-pulse z-0 right-6 "/>

                          ) :(
                            <Image src={reactjs} alt="next image" className=" h-[60px] w-[60px] absolute 
                            object-cover text-4xl text-gray bottom-6 animate-pulse z-0 right-6 "/>
                  ...
                  border-neutral-500 border-[2px] shadow-card  z-10 p-5 min-h-[560px]  bg-cover rounded-3xl`}>
                            ...
                            <div  style={{animationDuration:"3s"}}  className="black-gradient animate-spin duration-1000 absolute right-2 top-2 p-2 rounded-full " >
                              <Image src={github} className="h-10 w-10" height={50} width={50}  alt="" />
                            </div> 
            ...
            py-56 backdrop-blur-lg bottom-36 z-40 rounded-xl   pointer-events-none  
             ...
             focus:ring-2 focus:ring-slate-400 text-base   z-50 2  text-white
              font-semibold h-12 px-6 rounded-lg flex items-center
               pointer-events-auto">Show more...</button>
           </>
          }
        </div>
        </>
